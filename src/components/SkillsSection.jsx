...
const skillsData = [
  ...
  },
  {
    ...
    skills: [
      ...
      { name: 'FPGA/Verilog', percentage: '50%' },
...
const SkillsSection = () => {
  ...
  return (
    <section
      id="skills"
      className="bg-dark-theme-bg text-dark-theme-text py-16 px-8 md:px-16 lg:px-24 min-h-screen flex items-center justify-center">
      <div
        ...
        <div
          style={{ opacity: skillCardsOpacity }}
          className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
          {skillsData.map(card => (
            <div
              key={card.title}
              className="bg-gray-800 p-8 rounded-lg shadow-lg border border-gray-700 hover:border-gradient-start transition-colors duration-300">
        ...
        <div
          ref={otherSkillsCard}
          style={{ opacity: otherSkillsOpacity }}
          className="bg-gray-800 p-8 rounded-lg shadow-lg border border-gray-700 hover:border-gradient-start transition-colors duration-300">
          <div className="flex items-center mb-6">
          ...
          <p className="text-gray-300 leading-relaxed mb-6">
            In addition to my core skills, I have a diverse set of
            expertise that enhances my ability to contribute effectively in
            various domains. Here are some of the key areas where I excel.
          </p>
          <div className="flex flex-wrap gap-4">
            {expertiseData.map(tag => (
              <span
                key={tag}
                className="px-4 py-2 rounded-full bg-gray-700 text-sm font-semibold">
                {tag}
              </span>
            ))}
          </div>
